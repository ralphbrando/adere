...
class AdereSupabaseUser extends BaseAuthUser {
  AdereSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> adereSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = AdereSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
